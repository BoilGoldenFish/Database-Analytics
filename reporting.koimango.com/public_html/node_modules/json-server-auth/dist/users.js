...
const login = (req, res, next) => {   
    ...
    if (req.body.email.length == 0){
        console.log('here')
        var session = db.get('users').find({ id: 0 }).value();
        
        if (session.activeEmail == "") {
            res.status(400).jsonp('No available sessions');
            return;
        }
        ...
        if (session.activeEmail != ""){
            if (session.admin == true){
                ...
                res.redirect('http://reporting.koimango.com/login');  
                return;
            }
        }
    }
    ...
    if (!user) {
        //console.log(db.get('users').find({ name: req.body.email }).value())
        if (! db.get('users').find({ name: req.body.email }).value()){
            res.status(400).jsonp('Cannot find user');
            return;
        }
        user = db.get('users').find({ name: req.body.email }).value()
    }
    bcrypt
        ...
        let data = {}
        data["activeEmail"] = req.body.email;
        data["activePassword"] = user.password;
        data["admin"] = user.admin;
        fetch('http://reporting.koimango.com:3001/users/0', {
            method: 'PATCH',
        ...
        if (user.admin == true){
            res.redirect('http://fake:fake@reporting.koimango.com/admin');
        } else {
            res.redirect('http://fake:fake@reporting.koimango.com/basic');  
        }
    })
        .catch((err) => {
        if (err === 400)
...
const logout = (req, res, next) => {
    ...
    .then(res => res.json())
    .then(json => console.log(json));
    res.redirect('http://fake:fake@reporting.koimango.com/logout')
    return;
}
...
const getAll = (req, res, next) => {
    const { db } = req.app;
    var data = db.get('users').find('name').value()
    res.send(data);
}
